...
class Downloader:
            ...
            try:
                img_data = self.Load_json_by_lists(html[:html.index("\n") - 1],
                                                   ["contents", "twoColumnSearchResultsRenderer"
                                                    ...
                                                    "itemSectionRenderer", "contents", i, "videoRenderer", "thumbnail",
                                                    "thumbnails", 3, "url"])
            except KeyError:
                pass
            else:
    ...
    def Download(self, tag, Is_index=False, **args):
        ...
        file_name = args["filename"] if "filename" in args else self.site_data["title"] + "." + \
                                                                re.split('[/;="]', self.video_list[index]["mimeType"])[
                                                                    1]
...
if __name__ == "__main__":
    ...
    def downloading(args):
        ...
        print("\rダウンロード中[{3:-<50}] {0:>10}/{1}Byte {4:>5}% eta {2:>4}".format(downloaded_bytes, length, eta_t, "▉" * lo,
                                                                              parsent), end="")
